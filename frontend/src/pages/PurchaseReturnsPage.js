...
export default function PurchaseReturnsPage() {
  ...
  const [purchaseDetail, setPurchaseDetail] = useState(null); // lots from selected purchase
  ...
  const load = () => purchaseReturnsAPI.list().then(r => setReturns(r.data));

  useEffect(() => {
    load();
    purchasesAPI.list().then(r => setPurchases(r.data));
  }, []);

  // When purchase is selected in form, load its lots
  const handlePurchaseSelect = async (purchaseId) => {
    setForm(f => ({ ...f, purchase_id: purchaseId }));
    setItems([]);
  ...
  const handleCreate = async (e) => {
    ...
    try {
      await purchaseReturnsAPI.create({ ...form, items: returnItems });
      toast.success("Purchase return recorded");
      setShowForm(false);
      setForm({ purchase_id: '', date: new Date().toISOString().split('T')[0], reason: '', notes: '' });
      setItems([]);
      setPurchaseDetail(null);
      load();
    } catch (err) {
  ...
  const viewDetail = async (id) => {
    const { data } = await purchaseReturnsAPI.get(id);
    setSelected(data);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Return stock to suppliers</p>
        </div>
        <button
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="industrial-card p-5 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
            <div>
              <label className="block text-xs text-slate-400 uppercase mb-1" style={{ fontFamily: 'Barlow Condensed' }}>Purchase Order *</label>
              <select
              ...
              <div className="space-y-2 mb-4">
                {items.map((item, idx) => (
                  <div key={idx} className="grid grid-cols-4 gap-3 items-center bg-[#0A0F1C] border border-[#2D3648] rounded-sm px-3 py-2">
                    ...
                    <div>
                      <p className="text-xs text-slate-500">Original Qty</p>
          ...
          {form.purchase_id && items.length === 0 && (
            <p className="text-slate-500 text-sm mb-4">Loading purchase items...</p>
          )}
          ...
          <div className="flex gap-2 mt-2">
            ...
            <button type="button" onClick={() => { setShowForm(false); setPurchaseDetail(null); setItems([]); }}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className={`${selected ? 'lg:col-span-2' : 'lg:col-span-3'} industrial-card`}>
          <table className="w-full erp-table">
            <thead>
              <tr>
                <th>Return #</th><th>Purchase #</th><th>Supplier</th><th>Date</th><th>Amount</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {returns.map(r => (
                <tr key={r.id} onClick={() => viewDetail(r.id)} className="cursor-pointer">
                  ...
                  <td className="text-slate-400 font-mono text-xs">{r.purchase_number}</td>
                  <td className="text-slate-200">{r.supplier_name}</td>
        ...
        {selected && (
          <div className="industrial-card p-5 animate-slide-in">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between"><span className="text-slate-500">Purchase #</span><span className="text-slate-300 font-mono">{selected.purchase_number}</span></div>
              <div className="flex justify-between"><span className="text-slate-500">Supplier</span><span className="text-white">{selected.supplier_name}</span></div>
